...
/datum/element/unique_examine/proc/get_note_message(mob/examiner)
	switch(desc_requirement)
		...
		if(EXAMINE_CHECK_JOB)
			...
			if(!(their_job))
				return
			for(var/datum/job/job_datum_type as anything in requirements)
				if(their_job != job_datum_type)
					return
		...
		if(EXAMINE_CHECK_DEPARTMENT)
			...
			var/their_department = examiner.mind.assigned_role
			if(!their_department)
				return
...
/datum/element/unique_examine/proc/get_identifier(atom/thing)
	...
	if(ishuman(thing))
		return "person"
	if(isanimal(thing)) // Add a basic mob check here if we get them.
		return "animal"
	if(ismob(thing))
		return "creature"
...
/datum/element/unique_examine/proc/get_department(var/datum/job/our_job)
	var/department_text = "on the station"

	if(our_job.exp_type_department == EXP_TYPE_COMMAND) // As we don't have department bitflags, I've had to hijack department experience for this. Replace with bitflags if we get them.
		department_text = "as a member of command staff"
	if(our_job.exp_type_department == EXP_TYPE_SECURITY)
		department_text = "as a member of security force"
	if(our_job.exp_type_department == EXP_TYPE_SERVICE)
		department_text = "in the service department"
	if(our_job.exp_type_department == EXP_TYPE_SUPPLY)
		department_text = "in the cargo bay"
	if(our_job.exp_type_department == EXP_TYPE_ENGINEERING)
		department_text = "as one of the engineers"
	if(our_job.exp_type_department == EXP_TYPE_SCIENCE)
		department_text = "in the science team"
	if(our_job.exp_type_department == EXP_TYPE_MEDICAL)
		department_text = "in the medical field"
	if(our_job.exp_type_department == EXP_TYPE_SILICON)
